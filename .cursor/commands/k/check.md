...
**合并更新规则**：
1. **以 spec 为主**：用 spec 内容更新核心需求部分
2. **流程图更新**：如流程变化，直接修改原流程图
3. **删除冗余**：移除过时内容（见上方"必须删除"列表）
4. **保留分析**：保留有价值的实现思路和历史分析

### 情况 B：没有相关文档 → 新建合并文档

如果 history 目录中**没有**相关文档，**新建合并文档**：
- 命名格式：`YYYYMMDD-feature-description.md`（全英文，功能描述限5个单词）
- 内容：合并 spec 需求 + 本次实现的流程图和核心逻辑
